...
# objects.  Both implement the same rule: lowercase + spaces Ã¢ÂÂ underscores.
# ---------------------------------------------------------------------------
...
def _owner_username_slug(name: str) -> str:
    ...
    Example: "John Doe" Ã¢ÂÂ "john_doe"
    """
...
def create_podcast(db: Session, podcast: schemas.PodcastCreate, owner_id: int) -> models.Podcast:
    ...
    # immediately Ã¢ÂÂ without blocking on potentially many push/DB writes.
...
def _notify_followers_new_episode(
    ...
    Private podcasts are silently skipped Ã¢ÂÂ followers must not learn about
    ...
    """
    # Do not notify for private episodes Ã¢ÂÂ leaks metadata to followers.
    ...
    # Bulk insert Ã¢ÂÂ single commit instead of one commit per follower.
    notifications = [
        models.Notification(
            user_id=fid,
            type="new_episode",
...
def notify_followers_new_episode_background(podcast_id: int) -> None:
    ...
    ``owner_id`` is intentionally NOT a parameter Ã¢ÂÂ it is read from the
    ...
    try:
        ...
        if podcast is None:
            ...
            return
        # Derive owner_id from the persisted row Ã¢ÂÂ single source of truth.
        _notify_followers_new_episode(db=db, podcast=podcast, owner_id=podcast.owner_id)
    except Exception as exc:  # pragma: no cover Ã¢ÂÂ network / DB failures in background
...
def _safe_create_notification(db: Session, **kwargs) -> None:
    """
    Create a notification without raising Ã¢ÂÂ notification failures must never
...
def like_podcast(db: Session, user_id: int, podcast_id: int) -> models.PodcastLike:
    ...
    # Ã¢ÂÂÃ¢ÂÂ Notification: notify the podcast owner (skip self-likes) Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
    podcast = db.query(models.Podcast).filter(models.Podcast.id == podcast_id).first()
...
def create_comment(db: Session, comment: schemas.PodcastCommentCreate, user_id: int) -> models.PodcastComment:
    ...
    # Ã¢ÂÂÃ¢ÂÂ Notification: notify the podcast owner (skip self-comments) Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
    podcast = db.query(models.Podcast).filter(models.Podcast.id == comment.podcast_id).first()
    if podcast and podcast.owner_id != user_id:
        actor = db.query(models.User).filter(models.User.id == user_id).first()
        actor_name = actor.name if actor else "Someone"
        preview = comment.content[:60] + ("Ã¢ÂÂ¦" if len(comment.content) > 60 else "")
        _safe_create_notification(
            db,
            user_id=podcast.owner_id,
            type="comment",
            title="New Comment Ã°ÂÂÂ¬",
...
def get_recommended_podcasts(db: Session, user_id: int, limit: int = 10):
    ...
    # Core select() + union() Ã¢ÂÂ this avoids the legacy db.query() coercion
...
def get_user_playlists(
    ...
    - preview_thumbnails is a list of up to 4 thumbnail_url strings from the
      first items in each playlist (by position), used for the 2ÃÂ2 mosaic UI.
    ...
    # the rows we actually need (no N+1 Ã¢ÂÂ no Python-side truncation).
...
def get_public_playlists(
    ...
    Args:
        ...
        skip: Number of records to skip
        limit: Maximum number of records
        q: Optional search query Ã¢ÂÂ filters by playlist name, owner display name, or owner username slug (name lowercased with spaces replaced by underscores). Case-insensitive.
...
def send_direct_message(
    ...
    # Ã¢ÂÂÃ¢ÂÂ Notify recipient of incoming DM (in-app + push) Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
    ...
    try:
        ...
        preview = body[:80] + ("Ã¢ÂÂ¦" if len(body) > 80 else "")
        create_notification(
            db=db,
...
def mark_conversation_read(
    ...
    """
    Mark all unread messages from partner_id Ã¢ÂÂ reader_id as read.
...
# Ã¢ÂÂÃ¢ÂÂ Device Token / Push Notification CRUD Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
...
def _send_expo_push(
    ...
    """
    Best-effort synchronous Expo Push API call.  Never raises Ã¢ÂÂ failures are logged.
        ...
        try:
            ...
            try:
                ...
                for idx, ticket in enumerate(result.get("data", [])):
                    ...
                    if status == "error":
                        logger.warning(
                            "Expo Push ticket error Ã¢ÂÂ details: %s",
...
def check_push_receipts(
    ...
    For each receipt:
    - status "ok"  Ã¢ÂÂ ticket resolved successfully; delete the row.
    - status "error" + error "DeviceNotRegistered" Ã¢ÂÂ delete the device_token
      row (Expo confirmed the token is dead) and the ticket row.
    - status "error" (other) Ã¢ÂÂ log the error and delete the ticket row.
    ...
    Never raises Ã¢ÂÂ all errors are logged and a partial summary is returned.
    """
    ...
    # delete rows older than 25 hours Ã¢ÂÂ Expo receipts expire after 24 h.
    ...
    return {
        ...
        # returned yet Ã¢ÂÂ see receipts_returned for the count Expo actually
...
def search_users(
    ...
    Args:
        ...
        current_user_id: Optional authenticated user Ã¢ÂÂ used to populate
