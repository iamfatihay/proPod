...
# objects.  Both implement the same rule: lowercase + spaces ÃÂ¢ÃÂÃÂ underscores.
# ---------------------------------------------------------------------------
...
def _owner_username_slug(name: str) -> str:
    ...
    Example: "John Doe" ÃÂ¢ÃÂÃÂ "john_doe"
    """
...
def create_podcast(db: Session, podcast: schemas.PodcastCreate, owner_id: int) -> models.Podcast:
    ...
    # immediately ÃÂ¢ÃÂÃÂ without blocking on potentially many push/DB writes.
...
def _notify_followers_new_episode(
    ...
    Private podcasts are silently skipped ÃÂ¢ÃÂÃÂ followers must not learn about
    ...
    """
    # Do not notify for private episodes ÃÂ¢ÃÂÃÂ leaks metadata to followers.
    ...
    # Bulk insert ÃÂ¢ÃÂÃÂ single commit instead of one commit per follower.
    notifications = [
        models.Notification(
            user_id=fid,
            type="new_episode",
...
def notify_followers_new_episode_background(podcast_id: int) -> None:
    ...
    ``owner_id`` is intentionally NOT a parameter ÃÂ¢ÃÂÃÂ it is read from the
    ...
    try:
        ...
        if podcast is None:
            ...
            return
        # Derive owner_id from the persisted row ÃÂ¢ÃÂÃÂ single source of truth.
        _notify_followers_new_episode(db=db, podcast=podcast, owner_id=podcast.owner_id)
    except Exception as exc:  # pragma: no cover ÃÂ¢ÃÂÃÂ network / DB failures in background
...
def _safe_create_notification(db: Session, **kwargs) -> None:
    """
    Create a notification without raising ÃÂ¢ÃÂÃÂ notification failures must never
...
def like_podcast(db: Session, user_id: int, podcast_id: int) -> models.PodcastLike:
    ...
    # ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Notification: notify the podcast owner (skip self-likes) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
    podcast = db.query(models.Podcast).filter(models.Podcast.id == podcast_id).first()
    if podcast and podcast.owner_id != user_id:
        ...
        # Send Expo push notification to the podcast owner (best-effort)
        try:
            push_tokens = (
                db.query(models.DeviceToken.token)
                .filter(models.DeviceToken.user_id == podcast.owner_id)
                .all()
            )
            ...
            if token_strings:
                _send_expo_push(
                    tokens=token_strings,
                    title="New Like \u2764\ufe0f",
                    body=f"{actor_name} liked your podcast \"{podcast.title}\"",
                    data={"type": "like", "actorId": user_id, "podcastId": podcast_id},
                    db=db,
                )
        except Exception as exc:
            logger.warning("Like push notification failed (non-fatal): %s", exc)
...
def delete_listening_history(db: Session, user_id: int, podcast_id: int) -> bool:
    """
    Delete a single listening history entry for a user/podcast pair.

    Args:
        db: Database session
        user_id: User ID
        podcast_id: Podcast ID to remove from history

    Returns:
        bool: True if deleted, False if entry did not exist
    """
    history = db.query(models.ListeningHistory).filter(
        models.ListeningHistory.user_id == user_id,
        models.ListeningHistory.podcast_id == podcast_id,
...
def create_comment(db: Session, comment: schemas.PodcastCommentCreate, user_id: int) -> models.PodcastComment:
    ...
    # ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Notification: notify the podcast owner (skip self-comments) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
        ...
        try:
            ...
            if token_strings:
                _send_expo_push(
                    tokens=token_strings,
                    title="New Comment \U0001f4ac",
                    body=f"{actor_name} commented on \"{podcast.title}\": {preview}",
                    data={"type": "comment", "actorId": user_id, "podcastId": comment.podcast_id},
                    db=db,
                )
        except Exception as exc:
            logger.warning("Comment push notification failed (non-fatal): %s", exc)
...
def get_recommended_podcasts(db: Session, user_id: int, limit: int = 10):
    ...
    # Core select() + union() ÃÂ¢ÃÂÃÂ this avoids the legacy db.query() coercion
...
def get_user_playlists(
    ...
    - preview_thumbnails is a list of up to 4 thumbnail_url strings from the
      first items in each playlist (by position), used for the 2ÃÂÃÂ2 mosaic UI.
    ...
    # the rows we actually need (no N+1 ÃÂ¢ÃÂÃÂ no Python-side truncation).
...
def get_public_playlists(
    ...
    Args:
        ...
        skip: Number of records to skip
        limit: Maximum number of records
        q: Optional search query ÃÂ¢ÃÂÃÂ filters by playlist name, owner display name, or owner username slug (name lowercased with spaces replaced by underscores). Case-insensitive.
...
def send_direct_message(
    ...
    # ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Notify recipient of incoming DM (in-app + push) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
    ...
    try:
        ...
        preview = body[:80] + ("ÃÂ¢ÃÂÃÂ¦" if len(body) > 80 else "")
        create_notification(
            db=db,
...
def mark_conversation_read(
    ...
    """
    Mark all unread messages from partner_id ÃÂ¢ÃÂÃÂ reader_id as read.
...
# ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Device Token / Push Notification CRUD ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
...
def _send_expo_push(
    ...
    """
    Best-effort synchronous Expo Push API call.  Never raises ÃÂ¢ÃÂÃÂ failures are logged.
        ...
        try:
            ...
            try:
                ...
                for idx, ticket in enumerate(result.get("data", [])):
                    ...
                    if status == "error":
                        logger.warning(
                            "Expo Push ticket error ÃÂ¢ÃÂÃÂ details: %s",
...
def check_push_receipts(
    ...
    For each receipt:
    - status "ok"  ÃÂ¢ÃÂÃÂ ticket resolved successfully; delete the row.
    - status "error" + error "DeviceNotRegistered" ÃÂ¢ÃÂÃÂ delete the device_token
      row (Expo confirmed the token is dead) and the ticket row.
    - status "error" (other) ÃÂ¢ÃÂÃÂ log the error and delete the ticket row.
    ...
    Never raises ÃÂ¢ÃÂÃÂ all errors are logged and a partial summary is returned.
    """
    ...
    # delete rows older than 25 hours ÃÂ¢ÃÂÃÂ Expo receipts expire after 24 h.
    ...
    return {
        ...
        # returned yet ÃÂ¢ÃÂÃÂ see receipts_returned for the count Expo actually
...
def search_users(
    ...
    Args:
        ...
        current_user_id: Optional authenticated user ÃÂ¢ÃÂÃÂ used to populate
